...
# with open('data/AB-reprojected.json') as response:
#     ab = json.load(response)
...
mapbox_access_token_2="pk.eyJ1IjoiYW9pZGgiLCJhIjoiY2p6d3ZlZnhuMHFtczNpcWlsM3pqbjZicCJ9.FOWwd5ipYbL_9Kf670Loug"
df_ca = pd.read_csv(
    os.path.join(APP_PATH, os.path.join("data", "ca.csv"))
)
df_ab = pd.read_csv(
    os.path.join(APP_PATH, os.path.join("data", "ab_centroid.csv"))
)
dff_ab = pd.read_csv(
    os.path.join(
        APP_PATH, os.path.join("data", "yieldAB 2014-2018.csv")
    )
)
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            id="g-container",
            children=[
                html.P(id="c-selector", children="Select chart:"),
                dcc.Dropdown(
                    options=[
                        {
                            "label": "Canola Dryland WEIGHTED AVERAGE YIELD (single year)",
                            "value": "canola_single",
                        },
                    ],
...
def update(year):
    ...
    layout["mapbox"]["layers"].append(ab_layer)
    ...
    layout["mapbox"]["layers"].append(raster_layer)
...
@app.callback(
    ...
    [
        Input("dcc_oe_2", "selectedData"),
        Input("c-dropdown", "value"),
        Input("y-slider", "value")
    ]
)
def update_graph(selectedData, chart_dropdown, year):
    # Canola Dryland TOTAL ACREAGE
    # 2014 - 2018
    if selectedData is None:
        return dict(
            data=[dict(x=0, y=0)],
            layout=dict(
                title="Click-drag on the map to select riskzone",
                paper_bgcolor="#1f2630",
                plot_bgRcolor="#1f2630",
    ...
    # filter with rzs, using the second csv
    # get the rzs, but not year yet
    dff = dff_ab[dff_ab["riskzone"].isin(rzs)]
    dff = dff.sort_values("year")

    # replace the empty column, only for Canola Dryland WEIGHTED AVERAGE YIELD
    ...
    # it is a test, only the option Canola Dryland WEIGHTED AVERAGE YIELD
    AGGREGATE_BY = "Canola Dryland TOTAL ACREAGE "
    title = "Canola in a single year {0}".format(year)
    if chart_dropdown == "canola_single":
